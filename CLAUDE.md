...
Rules to keep:
- Rhetoric (evasion, fallacy) and conflicts with earlier claims (`contradicts`, from the `[E…]` lines) come from the same detection call; keep them conservative and never about motives.
...
Three blocks: media (with claim alerts over the video) and the Claims panel (Now / Checked / Other tabs, reviewer Confirm/Dispute/Dismiss + note, export .md/.json) on the left; the live transcript (search, clickable highlights) on the right. Sources sit behind a header popover. Reviews live in memory only; export to keep them.
